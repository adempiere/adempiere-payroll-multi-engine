 ...
 * getProcess() program is free software; you can redistribute it and/or modify it    *
 * under the terms version 2 or later of the                                  *
 * GNU General Public License as published                                    *
 * by the Free Software Foundation. getProcess() program is distributed in the hope   *
 * that it will be useful, but WITHOUT ANY WARRANTY; without even the implied *
 * warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.           *
 * See the GNU General Public License for more details.                       *
 * You should have received a copy of the GNU General Public License along    *
 * with getProcess() program; if not, write to the Free Software Foundation, Inc.,    *
 * 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA.                     *
 * For the text or an alternative of getProcess() public license, you may reach us    *
...
public class DefaultPayrollEngine implements PayrollEngine {
	...
	public int getHR_Job_ID() {
		return getProcess().getHR_Job_ID();
	}

	public Timestamp getDateAcct() {
		return getProcess().getDateAcct();
	}

	public int getHR_Payroll_ID() {
		return getProcess().getHR_Payroll_ID();
	}

	public int getHR_Period_ID() {
		return getProcess().getHR_Period_ID();
	}
	
	public int getHR_Department_ID() {
		return getProcess().getHR_Department_ID();
	}
	
	public int getAD_Org_ID() {
		return getProcess().getAD_Org_ID();
	}
	
	public int getHR_Process_ID() {
		return getProcess().getHR_Process_ID();
	}
	
	public String get_TrxName() {
		return getProcess().get_TrxName();
	}
	
	public int getC_Currency_ID() {
		return getProcess().getC_Currency_ID();
	}
	
	public int getC_ConversionType_ID() {
		return getProcess().getC_ConversionType_ID();
	}
	
	public int getAD_Client_ID() {
		return getProcess().getAD_Client_ID();
	}
		...
		try {
			...
			if(rule.isRuleClassGenerated()) {
				try {
					...
					if(ruleEngine != null) {
						isRunned = true;
						result = ruleEngine.run(getProcess(), scriptCtx);
	...
	private MHRMovement createMovementForCostCollector(int partnerId, MPPCostCollector costCollector)
	{
		...
		if (MHRConcept.TYPE_RuleEngine.equals(concept.getType())) {
			...
			//create movement
			MHRMovement movement = new MHRMovement(getProcess(), concept);
			...
			int bpGroupId = DB.getSQLValue(get_TrxName(), "SELECT C_BP_Group_ID FROM C_BPartner WHERE C_BPartner_ID=?", getProcess().partnerId);
	...
	*/
	...
	public MHRMovement createMovement(MHRConcept concept, BigDecimal qty , BigDecimal amount , String referenceNo , String description)
	{
		...
		MHRMovement.findByProcessAndConceptIdAndPartnerId(getProcess(), concept.getHR_Concept_ID() , partnerId, referenceNo , description)
				.stream()
	...
	public MHRAttribute getAttributeInstance(MHRConcept concept, int partnerId, Timestamp breakDate) {
		...
		//	BPartner
		if(partnerId <= 0)
			partnerId = getProcess().partnerId;
	...
	/**
	 * Get Concept for getProcess() payroll for range dates
	 * @param conceptValue
	 * @param from
	 * @param to
	 * @return
	...
	public MHREmployee setEmployee(String partnerValue,String  conceptValue)
	{
		...
		//Remove movement if getProcess() is calculated getProcess() way can be calculated again
		if (!concept.isManual())
